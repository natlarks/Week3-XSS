...
if(is_post_request()) {
  ...
  if($result === true) {
    redirect_to('show.php?id=' . urlencode(htmlspecialchars($user['id'])));
  } else {
    $errors = $result;
  }
}
?>
<?php $page_title = 'Staff: Edit User ' . htmlspecialchars($user['first_name']) . " " . urlencode(htmlspecialchars($user['last_name'])); ?>
<?php include(SHARED_PATH . '/header.php'); ?>

<div id="main-content">
  <a href="index.php">Back to Users List</a><br />

  <h1>Edit User: <?php echo urlencode(htmlspecialchars($user['first_name'])) . " " . urlencode(htmlspecialchars($user['last_name'])); ?></h1>
