<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if(preg_match('/\A[A-Za-z\s]+\Z/', $state['name'])===0){
    ...
    //My custom validation
    if(preg_match('/^[A-Z]{2}$/', $state['code'])===0){
        $errors[] = "Code must only be two capital letters.";
    }
    ...
    //My custom validation
    if(preg_match('/^[0-9]{1,}$/', $state['country_id'])===0){
        $errors[] = "Country ID must only be integers";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    //My custom validation
    if(preg_match('/^[0-9]{1,}$/', $territory['position'])===0){
        $errors[] = "Position must only be integers";
    }
    return $errors;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    //My custom validation
    if(preg_match('/\A[A-Za-z\s]+\Z/', $salesperson['first_name'])===0){
    ...
    //My custom validation
    if(preg_match('/\A[A-Za-z\s]+\Z/', $salesperson['last_name'])===0){
        $errors[] = "Last name must be letters only.";
    }

    if (is_blank($salesperson['email'])) {
      $errors[] = "Email cannot be blank.";
    } elseif (!filter_var($salesperson['email'], FILTER_VALIDATE_EMAIL)) {
    ...
    if(preg_match('/^\(?[0-9]{3}\)?\s?-?[0-9]{3}\s?-?[0-9]{4}$/', $salesperson['phone'])===0){
        $errors[] = "Phone number must only contain 0-9, spaces, and parantheses and dashes.";
    }
  ...
  function validate_user($user, $errors=array()) {
    ...
    if(preg_match('/\A[A-Za-z0-9\_]+\Z/', $user['username'])===0){
        $errors[] = "Username must only contain uppercase letters, lower case letters, numbers, or underscores.";
    }
    return $errors;
  }
